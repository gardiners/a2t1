...
We have existing tab-delimeted data in `expression.tsv`. We can solve the question with an `awk` one-liner:
...
We have exisiting data `annotation.tsv`. We can answer the question with an `awk` one-liner:
...
Here `-F $'\t'` ensures that `awk` uses the tab character as the delimeter, as several of our `function` values have space characers in them. The `awk` expression `'$2 ~ /ase/'` applies the regular expression `ase` to the second field (`function`), as described by @Wunschiers2013 \S 13.4.1 . All matching lines are returned as output.
...
Again, `-F $'\t'` ensures that `awk` uses the tab character as the delimeter. This time the regular expression is matched against the third field (annotation). The regex matches either of two sequences enclosed by the parentheses.
...
1. `ggplot` our dataset. Map the x and y aesthetics to the deaths per 100,000 (continuous) and country (categorical) respectively.
2. Add a barplot geometry. Use the "identity" statistic as we have pre-summarised our data by summing the deatgs by country. Match the example bar fill colour and increase the vertical space between each bar.
3. Remove the x- and y-axis lables to match the example.
4. Set title.
5. Set subtitle
6. Set caption for sources.
7. Use `theme_minimal` to remove most of `ggplot`'s defauly styling.
8. Begin setting our own theme. Left-align the country lables.
